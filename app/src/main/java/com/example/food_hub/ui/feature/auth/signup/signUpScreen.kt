...
@Composable
fun signUpScreen(navController: NavController,viewModel:SingUpViewmodel= hiltViewModel()){
Box(modifier=Modifier.fillMaxSize()){
    ...
    when(uiState.value){
        ...
        is SingUpViewmodel.signupEvent.Error ->{
               errormassage.value="Failed"
            loading.value=false
        }
        is SingUpViewmodel.signupEvent.Loading->{
    ...
    LaunchedEffect(true){
        viewModel.navigationEvent.collectLatest {event->
            when(event){
                is SingUpViewmodel.SignupNavigationEvent.NavegationToHome ->{
                ...
                is SingUpViewmodel.SignupNavigationEvent.NavegationToLogin ->{
    ...
    Box(modifier = Modifier.weight(1f))
        ...
        Spacer(modifier = Modifier.size(16.dp))
    Text(text = stringResource(R.string.already_login),
        modifier = Modifier.
    padding(8.dp)
            .clickable {
                viewModel.onLogin()
